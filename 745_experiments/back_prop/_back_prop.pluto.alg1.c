...
/*@ begin Loop(
transform ScalarReplace(dtype='float', overwrite_arr='in_FP')
if ((D >= 1) && (M >= 1) && (X >= 1) && (Y >= 1)) {
  ...
  float out1, out2, out3, out4, in_FP1, filter_FP11, filter_FP3, filter_FP2;
  float filter_FP1, filter_FP10, filter_FP7, filter_FP6, filter_FP5, filter_FP4, filter_FP12, filter_FP9;
  float filter_FP8;
  if ((D>=1)&&(M>=1)&&(X>=1)&&(Y>=1)) {
    for (t1=0; t1<=M-1-3; t1=t1+4) {
      for (t2=0; t2<=D-1-2; t2=t2+3) {
        filter_FP1=filter_FP[(t1+2)][t2];
        filter_FP4=filter_FP[(t1+2)][(t2+1)];
        filter_FP2=filter_FP[(t1+1)][(t2+2)];
        filter_FP3=filter_FP[(t1+3)][(t2+1)];
        filter_FP5=filter_FP[(t1+3)][t2];
        filter_FP8=filter_FP[(t1+2)][(t2+2)];
        filter_FP6=filter_FP[t1][(t2+1)];
        filter_FP7=filter_FP[t1][t2];
        filter_FP9=filter_FP[(t1+1)][(t2+1)];
        filter_FP10=filter_FP[(t1+1)][t2];
        filter_FP11=filter_FP[t1][(t2+2)];
        filter_FP12=filter_FP[(t1+3)][(t2+2)];
        for (t3=0; t3<=Y-1; t3++ ) {
          for (t4=0; t4<=X-1; t4++ ) {
            out1=out[t3][t4][(t1+3)];
            out2=out[t3][t4][(t1+1)];
            ...
            out4=out[t3][t4][t1];
            {
              in_FP1=in_FP[t3][t4][t2];
              filter_FP7=filter_FP7+out4*in_FP1;
              filter_FP10=filter_FP10+out2*in_FP1;
              filter_FP1=filter_FP1+out3*in_FP1;
              filter_FP5=filter_FP5+out1*in_FP1;
            }
            {
              in_FP1=in_FP[t3][t4][(t2+2)];
              filter_FP11=filter_FP11+out4*in_FP1;
              filter_FP2=filter_FP2+out2*in_FP1;
              filter_FP8=filter_FP8+out3*in_FP1;
              filter_FP12=filter_FP12+out1*in_FP1;
            }
            {
              ;
              ;
            ...
            {
              in_FP1=in_FP[t3][t4][(t2+1)];
              filter_FP6=filter_FP6+out4*in_FP1;
              filter_FP9=filter_FP9+out2*in_FP1;
              filter_FP4=filter_FP4+out3*in_FP1;
              filter_FP3=filter_FP3+out1*in_FP1;
            }
          }
        }
        filter_FP[(t1+2)][t2]=filter_FP1;
        filter_FP[(t1+2)][(t2+1)]=filter_FP4;
        filter_FP[(t1+1)][(t2+2)]=filter_FP2;
        filter_FP[(t1+3)][(t2+1)]=filter_FP3;
        filter_FP[(t1+3)][t2]=filter_FP5;
        filter_FP[(t1+2)][(t2+2)]=filter_FP8;
        filter_FP[t1][(t2+1)]=filter_FP6;
        filter_FP[t1][t2]=filter_FP7;
        filter_FP[(t1+1)][(t2+1)]=filter_FP9;
        filter_FP[(t1+1)][t2]=filter_FP10;
        filter_FP[t1][(t2+2)]=filter_FP11;
        filter_FP[(t1+3)][(t2+2)]=filter_FP12;
      }
      for (; t2<=D-1; t2++ ) {
        filter_FP5=filter_FP[(t1+3)][t2];
        filter_FP1=filter_FP[(t1+2)][t2];
        ...
        for (t3=0; t3<=Y-1; t3++ ) {
          for (t4=0; t4<=X-1; t4++ ) {
            {
              in_FP1=in_FP[t3][t4][t2];
              filter_FP7=filter_FP7+out[t3][t4][t1]*in_FP1;
              filter_FP10=filter_FP10+out[t3][t4][(t1+1)]*in_FP1;
              filter_FP1=filter_FP1+out[t3][t4][(t1+2)]*in_FP1;
              filter_FP5=filter_FP5+out[t3][t4][(t1+3)]*in_FP1;
            }
            {
              ;
              ;
    ...
    for (; t1<=M-1; t1++ ) {
      for (t2=0; t2<=D-1; t2++ ) {
        ...
        for (t3=0; t3<=Y-1; t3++ ) {
          for (t4=0; t4<=X-1; t4++ ) {
            {
              filter_FP7=filter_FP7+out[t3][t4][t1]*in_FP[t3][t4][t2];
            }
            {
              ;
            }
