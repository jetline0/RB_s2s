...
/*@ begin Loop(
transform ScalarReplace(dtype='float', overwrite_arr='out')
if ((D >= 1) && (M >= 1) && (X >= 1) && (Y >= 1)) {
  ...
  float out1, in8, in9, in4, in5, in6, in7, in1;
  float in2, in3, filter1, filter3, filter2, in12, in10, in11;
  if ((D>=1)&&(M>=1)&&(X>=1)&&(Y>=1)) {
    for (t1=0; t1<=Y-1-3; t1=t1+4) {
      for (t2=0; t2<=X-1; t2++ ) {
        for (t3=0; t3<=D-1-2; t3=t3+3) {
          ...
          in3=in[t1][t2][t3];
          in10=in[(t1+3)][t2][t3];
          in11=in[(t1+1)][t2][(t3+1)];
          in12=in[(t1+3)][t2][(t3+1)];
          for (t4=0; t4<=M-1; t4++ ) {
            filter1=filter[t4][(t3+2)];
            filter2=filter[t4][t3];
            filter3=filter[t4][(t3+1)];
            {
              out1=out[(t1+2)][t2][t4];
              in9=in9+out1*filter2;
              in7=in7+out1*filter3;
              in8=in8+out1*filter1;
            }
            {
              out1=out[(t1+3)][t2][t4];
              in10=in10+out1*filter2;
              in12=in12+out1*filter3;
              in5=in5+out1*filter1;
            }
            {
              out1=out[t1][t2][t4];
              in3=in3+out1*filter2;
              in4=in4+out1*filter3;
              in6=in6+out1*filter1;
            }
            {
              ;
              ;
            ...
            {
              out1=out[(t1+1)][t2][t4];
              in2=in2+out1*filter2;
              in11=in11+out1*filter3;
              in1=in1+out1*filter1;
            }
          }
          in[(t1+1)][t2][(t3+2)]=in1;
          in[t1][t2][(t3+1)]=in4;
          ...
          in[t1][t2][t3]=in3;
          in[(t1+3)][t2][t3]=in10;
          in[(t1+1)][t2][(t3+1)]=in11;
          in[(t1+3)][t2][(t3+1)]=in12;
        }
        for (; t3<=D-1; t3++ ) {
          in10=in[(t1+3)][t2][t3];
          in9=in[(t1+2)][t2][t3];
          ...
          for (t4=0; t4<=M-1; t4++ ) {
            filter2=filter[t4][t3];
            {
              in9=in9+out[(t1+2)][t2][t4]*filter2;
            }
            {
              in10=in10+out[(t1+3)][t2][t4]*filter2;
            }
            {
              in3=in3+out[t1][t2][t4]*filter2;
            }
            {
              ;
              ;
            ...
            {
              in2=in2+out[(t1+1)][t2][t4]*filter2;
            }
          }
          in[(t1+3)][t2][t3]=in10;
          in[(t1+2)][t2][t3]=in9;
    ...
    for (; t1<=Y-1; t1++ ) {
      for (t2=0; t2<=X-1; t2++ ) {
        for (t3=0; t3<=D-1; t3++ ) {
          ...
          for (t4=0; t4<=M-1; t4++ ) {
            {
              in3=in3+out[t1][t2][t4]*filter[t4][t3];
            }
            {
              ;
            }
