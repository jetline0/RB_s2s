...
if ((D >= 1) && (M >= 1) && (X >= 1) && (Y >= 1)) {
  ...
  if ((D>=1)&&(M>=1)&&(X>=1)&&(Y>=1)) {
    for (t1=0; t1<=Y-1-3; t1=t1+4) {
      for (t2=0; t2<=X-1; t2++ ) {
        for (t3=0; t3<=D-1-2; t3=t3+3) {
          float scv_8, scv_9, scv_10, scv_11, scv_12, scv_13, scv_14, scv_15;
          float scv_16, scv_17, scv_18, scv_19;
          scv_8=in[(t1+1)][t2][(t3+2)];
          scv_9=in[t1][t2][(t3+1)];
          scv_10=in[(t1+2)][t2][t3];
          scv_11=in[(t1+1)][t2][t3];
          scv_12=in[(t1+2)][t2][(t3+2)];
          scv_13=in[(t1+3)][t2][(t3+2)];
          scv_14=in[(t1+2)][t2][(t3+1)];
          scv_15=in[t1][t2][(t3+2)];
          scv_16=in[t1][t2][t3];
          scv_17=in[(t1+3)][t2][t3];
          scv_18=in[(t1+1)][t2][(t3+1)];
          scv_19=in[(t1+3)][t2][(t3+1)];
          for (t4=0; t4<=M-1; t4++ ) {
            float scv_1, scv_2, scv_3, scv_4, scv_5, scv_6, scv_7;
            scv_1=out[(t1+2)][t2][t4];
            scv_2=out[(t1+3)][t2][t4];
            scv_3=out[t1][t2][t4];
            scv_4=filter[t4][(t3+2)];
            scv_5=filter[t4][t3];
            scv_6=out[(t1+1)][t2][t4];
            scv_7=filter[t4][(t3+1)];
            scv_16=scv_16+scv_3*scv_5;
            ;
            scv_11=scv_11+scv_6*scv_5;
            ;
            scv_10=scv_10+scv_1*scv_5;
            ;
            scv_17=scv_17+scv_2*scv_5;
            ;
            scv_9=scv_9+scv_3*scv_7;
            ;
            scv_18=scv_18+scv_6*scv_7;
            ;
            scv_14=scv_14+scv_1*scv_7;
            ;
            scv_19=scv_19+scv_2*scv_7;
            ;
            scv_15=scv_15+scv_3*scv_4;
            ;
            scv_8=scv_8+scv_6*scv_4;
            ;
            scv_12=scv_12+scv_1*scv_4;
            ;
            scv_13=scv_13+scv_2*scv_4;
            ;
          }
          in[(t1+1)][t2][(t3+2)]=scv_8;
          in[t1][t2][(t3+1)]=scv_9;
          in[(t1+2)][t2][t3]=scv_10;
          in[(t1+1)][t2][t3]=scv_11;
          in[(t1+2)][t2][(t3+2)]=scv_12;
          in[(t1+3)][t2][(t3+2)]=scv_13;
          in[(t1+2)][t2][(t3+1)]=scv_14;
          in[t1][t2][(t3+2)]=scv_15;
          in[t1][t2][t3]=scv_16;
          in[(t1+3)][t2][t3]=scv_17;
          in[(t1+1)][t2][(t3+1)]=scv_18;
          in[(t1+3)][t2][(t3+1)]=scv_19;
        }
        for (; t3<=D-1; t3++ ) {
          float scv_21, scv_22, scv_23, scv_24;
          scv_21=in[(t1+3)][t2][t3];
          scv_22=in[(t1+2)][t2][t3];
          scv_23=in[(t1+1)][t2][t3];
          scv_24=in[t1][t2][t3];
          for (t4=0; t4<=M-1; t4++ ) {
            float scv_20;
            scv_20=filter[t4][t3];
            scv_24=scv_24+out[t1][t2][t4]*scv_20;
            ;
            scv_23=scv_23+out[(t1+1)][t2][t4]*scv_20;
            ;
            scv_22=scv_22+out[(t1+2)][t2][t4]*scv_20;
            ;
            scv_21=scv_21+out[(t1+3)][t2][t4]*scv_20;
            ;
          }
          in[(t1+3)][t2][t3]=scv_21;
          in[(t1+2)][t2][t3]=scv_22;
          in[(t1+1)][t2][t3]=scv_23;
          in[t1][t2][t3]=scv_24;
        }
      }
    }
    for (; t1<=Y-1; t1++ ) {
      for (t2=0; t2<=X-1; t2++ ) {
        for (t3=0; t3<=D-1; t3++ ) {
          ...
          scv_25=in[t1][t2][t3];
          for (t4=0; t4<=M-1; t4++ ) {
            scv_25=scv_25+out[t1][t2][t4]*filter[t4][t3];
            ;
          }
          in[t1][t2][t3]=scv_25;
        }
